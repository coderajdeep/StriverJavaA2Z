...
This is solved using the Subset Sum DP: find all achievable subset sums `s` in the range `[0, totalSum/2]`, and for each achievable sum, the corresponding difference is `abs(totalSum - 2*s)`. The minimum such difference is the answer.
